...
// TODO - don't setup and tear down high data rate each time? or allow it for longer readings



int parse_args(int argc, char* argv[], char **t_str, char **f_str, int *just_gps, char **p_str, int *use_high_sample_rate)
{
	int c;
	opterr = 0;
	char log_buffer[100];
...
int main(int argc, char* argv[])
{
	...
	if (parse_args(argc, argv, &t_str, &f_str, &just_gps, &p_str, &use_high_sample_rate) == FAILURE)
		return 1;
	...
	{
		char *end_ptr;
		max_runtime = strtof(p_str, &end_ptr);

		if (*end_ptr != 0 && !isspace((unsigned char)*end_ptr))
		{
		 write_to_log_and_stderr(LOG_FILENAME, "Invalid value given for duration string.\n", "a");
                 return 1;
...
// Register for updates
  gps_stream(&gps_data, WATCH_ENABLE | WATCH_JSON, NULL);
	...
	// Purge all the records that we accumulated whilst we wait for the higher data rate. This takes finite time
  ...
  int sleep_duration = (int)interval_time * SEC_TO_USEC;
	...
	while (is_running)
	{
		...
		for (int i=0; i < (TOTAL_RESULT_CHANNELS-1); i++)
		{
